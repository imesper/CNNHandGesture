...
def loadData(usePickle = True, loadFromFile = False):
    ...
    trainData = np.zeros((nTrainSamples, 1, nfileLines, nChannels), dtype="uint16")
    testData = np.zeros((nTestSamples, 1, nfileLines, nChannels), dtype="uint16")
    
    yTrain = np.zeros((nTrainSamples,), dtype="uint8")
    yTest = np.zeros((nTestSamples,), dtype="uint8")
